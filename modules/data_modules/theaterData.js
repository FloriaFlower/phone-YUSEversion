...
<div class="list-item" data-id="anno_12" data-type="announcement" data-title="【暗黑童话】《小红帽与三只狼》主角" data-description="颠覆童话，小红帽在森林里被三只“大灰狼”（温言、朝刃、顾麟）捕获。从被轮流口交到小穴被填满，在森林木屋里被连续三天三夜地操干，身体被彻底改造成离不开雄性精液的淫乱状态。需要极强的身体承受能力。" data-actor="朝温言, 朝刃, 顾麟" data-location="森林实景拍摄地" data-payment="片酬60万 + 30%平台分红">
  ...
  <div class="item-meta"><span>📍 森林实景拍摄地</span><span>🎬 温言, 朝刃, 顾麟</span><span class="item-tag">多人/童话改编</span><span class="item-price">💰 片酬60万 + 30%平台分红</span></div>
    ...
    customizations: `<div class="list-item" data-id="cust_1" data-type="customization" data-fan-id="霍" data-type-name="私人晚宴" data-request="下周五晚，在我私人府邸进行一场一对一的晚宴直播。服装由我提供，主题是'金丝雀的献礼'。直播内容很简单，只需要{{user}}全程穿着我指定的衣服，按照我的指示行动即可。不需要过度表演，自然就好。" data-deadline="下周五晚" data-payment="1,000,000" data-notes="一切开销由我承担，司机会在周五下午六点准时去接。">
...
  <div class="item-title">《恶犬饲养法则》</div>
    ...
    theater_hot: `<div class="list-item"
        data-id="th_hot_1"
        data-type="theater"
  ...
  <div class="item-title">《军魂：不可逾越的界线》</div>
    ...
    theater_new: `<div class="list-item"
        data-id="th_new_1"
        data-type="theater"
  ...
  <div class="item-title">【本周上新】《办公室秘情：总裁与实习生》</div>
    ...
    theater_recommended: `(<div class="list-item"
        data-id="th_rec_1"
        data-type="theater"
    ...
    <div class="list-item"
        ...
        data-description="主演：{{user}} (饰 独居白领-晴子), (特别出演)X (饰 邻居-？)。简介：这是一部伪纪录片形式的影片。晴子发现自己的生活似乎总被人窥探，家里的东西会莫名移动，内衣会无故失踪。直到有一天，晴子)提前回家，撞见了潜入家里的邻居。邻居并没有伤害晴子，只是逼晴子坐在椅子上，听他讲述自己是如何窥探晴子的一切，如何在晴子睡着时潜入房间，闻晴子的味道，甚至在晴子洗澡时躲在浴室的天花板上... 结局：晴子在极度的恐惧和羞耻中，被邻居的言语挑逗到高潮。而邻居只是微笑着看着晴子，留下一句“我一直在看着你”，然后消失在黑暗中。影片以晴子家中隐藏的无数个针孔摄像头的视角结束。"
    ...
    theater_paid: `<div class="list-item"
        data-id="th_paid_1"
        data-type="theater"
    ...
    shop: `<div class="list-item" data-id="shop_1" data-type="shop" data-name="【{{user}}原味】《沉沦的家庭教师》黑框眼镜" data-description="在《沉沦的家庭教师》中被朝刃操到歪掉的那副黑框眼镜。镜片上有已经干涸的可疑水渍，镜腿上有被汗水浸湿的痕迹。据说戴上它，就能感受到被年轻肉体支配的羞耻与快乐。" data-tags='["原味", "道具", "制服系列"]'
        data-price="¥5000"
...
// 【修改】只返回原始HTML数据，不内部解析
export async function fetchRawTheaterData() {
    try {
        const theaterDb = await DataHandler._fetchFromWorldbook(PhoneSim_Config.WORLD_THEATER_DATABASE);
        // 数据库有数据则返回数据库数据，否则返回默认HTML
        if (theaterDb && Object.keys(theaterDb).length > 0 && theaterDb.announcements) {
            return theaterDb;
        } else {
            return DEFAULT_THEATER_DATA;
        }
    } catch (e) {
        console.error('[Phone Sim] Error fetching raw theater data.', e);
        return DEFAULT_THEATER_DATA;
    }
}

// 【新增】暴露给外部的解析入口（由UI调用，传入parser的_parseListItems）
export function parseTheaterData(rawData, parseListItemsFn) {
    if (!parseListItemsFn || typeof parseListItemsFn !== 'function') {
        console.error('[Phone Sim] Missing parseListItems function.');
        return {};
    }
    // 调用外部传入的解析函数，避免内部依赖
    return {
        announcements: parseListItemsFn(rawData.announcements),
        customizations: parseListItemsFn(rawData.customizations),
        theater: parseListItemsFn(rawData.theater),
        theater_hot: parseListItemsFn(rawData.theater_hot),
        theater_new: parseListItemsFn(rawData.theater_new),
        theater_recommended: parseListItemsFn(rawData.theater_recommended),
        theater_paid: parseListItemsFn(rawData.theater_paid),
        shop: parseListItemsFn(rawData.shop)
    };
}

// saveTheaterData 保持不变（保存原始HTML）
export async function saveTheaterData(data, msgId) {
    ...
    // 通知UI刷新数据
    if (UI && UI.refreshTheaterView) UI.refreshTheaterView();
}
