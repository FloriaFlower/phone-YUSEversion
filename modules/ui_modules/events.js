...
export function addEventListeners() {
    ...
    p.on('click.phonesim', '#emaildetail-view .reply-button', async function() { PhoneSim_Sounds.play('tap'); const senderName = jQuery_API(this).data('sender-name'); const replyContent = await UI.showDialog(`回复 ${senderName}`); if (replyContent) { const prompt = `(系统提示：{{user}}回复了${senderName}的邮件：“${replyContent}”)`; await TavernHelper_API.triggerSlash(`/send ${JSON.stringify(prompt)} |/trigger`); UI.showView('EmailApp'); } });
    p.on('click.phonesim', '#emaildetail-view .accept-button', async function() { PhoneSim_Sounds.play('open'); const emailId = PhoneSim_State.activeEmailId; const email = PhoneSim_State.emails.find(e => e.id === emailId); if (email && email.attachment) { const prompt = `(系统提示：{{user}}收下了邮件“${email.subject}”中的附件“${email.attachment.name}”。)`; await TavernHelper_API.triggerSlash(`/send ${JSON.stringify(prompt)} |/trigger`); SillyTavern_Context_API.callGenericPopup(`已收下附件：${email.attachment.name}`, 'text'); } });
    ...
    p.on('click.phonesim', '.live-stream-item', async function() { PhoneSim_Sounds.play('open'); const streamerId = String(jQuery_API(this).data('streamer-id')); UI.showView('LiveStreamRoom', streamerId); const stream = DataHandler.findLiveStreamById(streamerId); if (stream) { const prompt = `(系统提示：{{user}}进入了 ${stream.streamerName} 的直播间“${stream.title}”，请生成当前的直播内容和弹幕。)`; await TavernHelper_API.triggerSlash(`/send ${JSON.stringify(prompt)} |/trigger`); } else { console.error(`[Phone Sim] Could not find stream data for streamerId: ${streamerId}`); } });
    const handleSearch = async (inputElement) => { const input = jQuery_API(inputElement); const searchTerm = input.val().trim(); if (!searchTerm) return; PhoneSim_Sounds.play('send'); let prompt = ''; const view = input.closest('.view'); if (view.is('#forumpostlist-view')) { const boardName = view.find('.app-header h3').text(); prompt = `(系统提示：{{user}}在论坛的“${boardName}”板块中搜索：“${searchTerm}”。请生成相关的帖子列表。)`; view.find('.forum-post-list-content').html(UI.getPostListSkeleton()); } else if (view.is('#livestreamlist-view')) { const boardName = view.find('.app-header h3').text(); prompt = `(系统提示：{{user}}在直播中心的“${boardName}”板块中搜索：“${searchTerm}”。请生成相关的直播列表。)`; view.find('.live-stream-list-content').html(UI.getStreamListSkeleton()); } if (prompt) { await TavernHelper_API.triggerSlash(`/send ${JSON.stringify(prompt)} |/trigger`); input.val(''); } };
    ...
    const sendDanmaku = () => {
        ...
        if (content && PhoneSim_State.activeLiveStreamId) {
            PhoneSim_Sounds.play('send');
            DataHandler.stagePlayerAction({ type: 'new_danmaku', streamerId: PhoneSim_State.activeLiveStreamId, content: content });
            input.val('');
        }
    };
    ...
    p.on('click.phonesim', '.reject-call', async () => { PhoneSim_Sounds.play('close'); SillyTavern_Context_API.stopGeneration(); if (PhoneSim_State.incomingCallData) { const prompt = `(系统提示：{{user}}拒绝了来自${PhoneSim_State.incomingCallData.name}的通话。)`; await TavernHelper_API.triggerSlash(`/send ${JSON.stringify(prompt)} |/trigger`); } UI.closeCallUI(); });
    p.on('click.phonesim', '.accept-call', async () => { PhoneSim_Sounds.play('open'); if (!PhoneSim_State.incomingCallData) return; const name = PhoneSim_State.incomingCallData.name; p.find('.voice-call-modal').hide().find('audio')[0].pause(); const prompt = `(系统提示：{{user}}接听了${name}的通话。)`; await TavernHelper_API.triggerSlash(`/send ${JSON.stringify(prompt)} |/trigger`); PhoneSim_State.incomingCallData = null; });
    ...
    p.on('click.phonesim', '.call-ui-internal .end-call', async function() {
        ...
        if (callData) {
            ...
            await DataHandler.logCallRecord({ contactId: callData.id, duration: timerText, timestamp: new Date().toISOString(), callType: isVoiceCall ? 'wechat' : 'phone' });
            if (isVoiceCall) { await DataHandler.addWeChatCallEndMessage(callData.id, timerText); }
            const genericPrompt = `(系统提示：与${contactName}的通话已结束。)`;
            await TavernHelper_API.triggerSlash(`/send ${JSON.stringify(genericPrompt)} |/trigger`);
        }
        UI.closeCallUI();
    });
    ...
    p.on('click.phonesim', '#generate-moment-btn', async () => { PhoneSim_Sounds.play('tap'); const prompt = `(系统提示：请为任意一位角色生成一条新的朋友圈动态。)`; await TavernHelper_API.triggerSlash(`/send ${JSON.stringify(prompt)} |/trigger`); });
    p.on('click.phonesim', '#homepage-view #generate-profile-update-btn', async () => { PhoneSim_Sounds.play('tap'); if (PhoneSim_State.activeProfileId) { const contactName = UI._getContactName(PhoneSim_State.activeProfileId); const prompt = `(系统提示：请为 ${contactName} 更新个人主页并生成一条新的动态。)`; await TavernHelper_API.triggerSlash(`/send ${JSON.stringify(prompt)} |/trigger`); } });
