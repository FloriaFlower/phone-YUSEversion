...
export function addEventListeners() {
    ...
    p.off('.phonesim')
        ...
        .on('click.phonesim', '.friend-request-btn', function() {
            const button = jQuery_API(this);
            const bubble = button.closest('.friend-request-bubble');
            const uid = bubble.data('uid');
            const from_id = bubble.data('from-id');
            const from_name = bubble.data('from-name');
            const action = button.data('action');

            if (uid && from_id && from_name && action) {
                 PhoneSim_Sounds.play('tap');
                 DataHandler.stageFriendRequestResponse(uid, action, from_id, from_name);
            } else {
                 console.error("Missing data for friend request action", {uid, from_id, from_name, action});
                 toastr.error("处理好友请求时发生错误。");
            }
        })
